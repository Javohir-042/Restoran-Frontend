...
export const CreateOrderModal = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#18181b] rounded-2xl w-full max-w-lg max-h-[85vh] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-[#27272a] shrink-0">
          <h2 className="text-base font-semibold text-gray-900 dark:text-[#fafafa]">
            {step === "table" ? t("Stolni tanlang") : t("Taomlarni tanlang")}
          </h2>
          <button
            onClick={onClose}
        ...
        {step === "table" && (
          <div className="p-5 overflow-y-auto">
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
              {tables.map((tableItem) => (
                <button
                  ...
                  disabled={loading}
                  className={`p-3 rounded-lg text-sm font-medium border transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${tableItem.status === "BAND"
                    ? "bg-orange-50 dark:bg-orange-500/10 border-orange-100 dark:border-orange-500/20 text-orange-600 dark:text-orange-400 hover:bg-orange-100 dark:hover:bg-orange-500/20"
                    : "bg-green-50 dark:bg-green-500/10 border-green-100 dark:border-green-500/20 text-green-600 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-500/20"
                    }`}
                >
                  {t("Stol")} {tableItem.tableNumber}
                  {tableItem.status === "BAND" && (
            ...
            {tables.length === 0 && (
              <p className="text-sm text-gray-400 dark:text-[#71717a] text-center py-8">
                {t("Stollar topilmadi")}
              </p>
            )}
          </div>
        ...
        {step === "items" && (
          <>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <button
                        ...
                        className="w-7 h-7 rounded-full border border-gray-200 dark:border-[#3f3f46] flex items-center justify-center text-gray-500 dark:text-[#a1a1aa] hover:border-gray-300 dark:hover:border-[#52525b] hover:bg-gray-50 dark:hover:bg-[#27272a] disabled:opacity-30 transition-colors"
                      >
                        <Minus size={13} />
                      </button>
                      <span className="w-6 text-center text-sm font-medium tabular-nums dark:text-[#fafafa]">
                        {cart[item.id] ?? 0}
                      </span>
                      <button
                        onClick={() => updateQty(item.id, 1)}
            ...
            {/* Footer */}
            <div className="px-5 py-4 border-t border-gray-100 dark:border-[#27272a] shrink-0">
              {totalPrice > 0 && (
                <p className="text-xs text-gray-500 dark:text-[#a1a1aa] mb-2">
                  {t("Jami:")}{" "}
              ...
              >
                {isSubmitting ? t("Saqlanmoqda...") : t("Buyurtmani saqlash")}
              </button>
            </div>
          </>
        )}
