...
export const ViewOrderModal = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#18181b] rounded-2xl w-full max-w-md max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-[#27272a] sticky top-0 bg-white dark:bg-[#18181b]">
          <h2 className="text-base font-semibold text-gray-900 dark:text-[#fafafa]">
            {t("Buyurtma")} #{order.bill.id.slice(0, 6).toUpperCase()}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-4">
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-gray-500 dark:text-[#a1a1aa]">{t("Stol")}</span>
            ...
            <div className="flex justify-between text-sm">
              <span className="text-gray-500 dark:text-[#a1a1aa]">{t("Xodim")}</span>
              <span className="font-medium text-gray-900 dark:text-[#fafafa]">
                {order.staffName === "Noma'lum" ? t("Noma'lum") : order.staffName}
              </span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-500 dark:text-[#a1a1aa]">{t("Holat")}</span>
              <span className="font-medium text-gray-900 dark:text-[#fafafa]">
                {order.bill.status}
              </span>
            </div>
          ...
          <div className="border-t border-gray-100 dark:border-[#27272a] pt-3 space-y-2">
            <p className="text-xs font-medium text-gray-500 dark:text-[#a1a1aa] mb-2">{t("Taomlar")}</p>
            {order.items.length === 0 ? (
              <p className="text-sm text-gray-400 dark:text-[#71717a]">Taom qo'shilmagan</p>
            ) : (
          ...
          <div className="border-t border-gray-100 dark:border-[#27272a] pt-3 space-y-1 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-500 dark:text-[#a1a1aa]">{t("Jami:")}</span>
