...
export const Settings = () => {
    ...
    const tabs = [
        ...
        { id: "serviceFee", label: "Xizmat haqi" },
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <div className="flex items-center gap-3">
                <div className="p-2.5 bg-blue-50 text-[#1a56db] rounded-xl shadow-sm border border-blue-100">
                    <SettingsIcon className="w-6 h-6" />
                </div>
                <div>
                    <h1 className="text-2xl font-bold tracking-tight text-gray-900">Sozlamalar</h1>
                    <p className="text-sm text-gray-500 mt-1">
                        Tizim parametrlarini boshqarish va moslashtirish
                    </p>
                </div>
            </div>

            {/* Main Card */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200/60 overflow-hidden ring-1 ring-black/[0.03]">
                {/* Tabs */}
                <div className="border-b bg-gray-50/50 px-4 sm:px-8 flex overflow-x-auto gap-6 sm:gap-8 no-scrollbar pt-6">
                    {tabs.map((tab) => (
                        <button
                            key={tab.id}
